...
function App() {
  ...
  useEffect(() => {
    const score = localStorage.getItem("rps");
    setMainScore(+score);
  }, []);
  const coinsInfo = [
    {
      containerClass: "paper coin",
      width: "49",
      height: "59",
  ...
  return (
    <>
      <div className="container">
        <div className="app">
          <TopScore mainScore={mainScore} />
          {!chosen && <Choose coinsInfo={coinsInfo} setChosen={setChosen} />}
          {chosen && !result && (
            <Battle
              coinsInfo={coinsInfo}
              chosen={chosen}
              setResult={setResult}
              setHousePlayed={setHousePlayed}
              mainScore={mainScore}
